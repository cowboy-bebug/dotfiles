...
** Stow
#+begin_src bash
stow --verbose --restow --no-folding --target="$HOME" */
#+end_src
...
*** Hooks
Claude Code rewrites ~settings.json~ every time it touches and it results in messy
diffs so normalising the key order here:
#+begin_src bash :tangle .git/hooks/pre-commit :tangle-mode (identity #o755) :mkdirp yes
#!/usr/bin/env bash
set -euo pipefail

f="claude/.claude/settings.json"
...
** Docker
Add this to ~~/.config/docker/config.json~ so =docker compose=, =docker buildx=, etc.
installed via Homebrew get discovered:
#+begin_src json :tangle ~/.config/docker/config.json :mkdirp yes
{
...
** CSpell
#+begin_src json :tangle ~/Library/Preferences/cspell/cspell.json :mkdirp yes
{
  "language": "en-GB"
}
#+end_src
